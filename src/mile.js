...
module.exports = mile = {
  ...
  createLayer: function (req, res) {
    ...
    // verify query
    if (!file_id)
      return mile.error.missingInformation(res, "Please provide a file_id.");
    ...
    ops.push(function (callback) {
      ...
      var layer = {
        ...
        options: {
          ...
          data_type:
            requested_layer.data_type || upload_status.data_type || "vector",
  ...
  getLayer: function (req, res) {
    ...
    if (!layerUuid)
      return mile.error.missingInformation(res, "Please provide layerUuid.");
  ...
  prepareTile: function (params, done) {
    ...
    ops.push(function (storedLayerJSON, callback) {
      ...
      var output_extent =
        output_a[0] + " " + output_b[0] + "," + output_a[1] + " " + output_b[1];
  ...
  lat_to_tile_y: function (lat, zoom) {
    return Math.floor(
      ((1 -
        Math.log(
          Math.tan(mile.deg_to_rad(lat)) + 1 / Math.cos(mile.deg_to_rad(lat))
        ) /
          Math.PI) /
        2) *
        Math.pow(2, zoom)
