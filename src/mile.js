...
module.exports = mile = {
    ...
    serveTile: function (req, res) {
        ...
        store.layers.get(params.layerUuid, function (err, storedLayerJSON) {
            ...
            if (!storedLayerJSON)
                return mile.tileError(res, "No stored layer.");
            ...
            async.series(ops, function (err, data) {
                ...
                if (type == "pbf") {
                    ...
                    return zlib.gzip(data[0], function (err, zipped) {
                        res.end(zipped);
                    });
                }
    ...
    createLayer: function (req, res) {
        ...
        // verify query
        if (!file_id)
            return mile.error.missingInformation(
                res,
                "Please provide a file_id."
            );
        ...
        ops.push(function (upload_status, callback) {
            ...
            var error_message =
                "The data is not done processing yet. Please try again in a little while.";
            if (!upload_status.processing_success)
    ...
    vectorizeRaster: function (data, done) {
        ...
        ops.query = function (callback) {
            ...
            var query =
                ...
                " FROM (SELECT (ST_DumpAsPolygons(rast)).* FROM " +
                raster_table_name +
                ") As foo ORDER BY " +
                column +
                ";";
        ...
        ops.columns = function (callback) {
            // get columns
            var query =
                "SELECT * FROM " + vectorized_raster_file_id + " LIMIT 1";
            queries.postgis(
                {
                    postgis_db: database_name,
                    query: query,
                ...
                function (err, results) {
                    ...
                    columns.forEach(function (column) {
                        ...
                        jobs.push(function (done) {
                            ...
                            var query =
                                "select row_to_json(t) from (select MAX(" +
                                column +
                                "), MIN(" +
                                column +
                                "), AVG(" +
        ...
        ops.status = function (callback) {
            ...
            upload_status.processing_took_ms =
                new Date().getTime() - upload_status.timestamp;
            upload_status.metadata = JSON.stringify(temp_meta);
            upload_status.sql =
                "(SELECT * FROM " + vectorized_raster_file_id + ") as sub";
        ...
        ops.push(function (callback) {
            ...
            var layer = {
                ...
                options: {
                    ...
                    table_name:
                        requested_layer.table_name || upload_status.table_name,
                    metadata: upload_status.metadata,
                    data_type:
                        requested_layer.data_type ||
    ...
    _renderVectorTile: function (params, done) {
        ...
        if (params.z == undefined)
            return done("Invalid url: Missing tile coordinates. z", params.z);
        if (params.x == undefined)
            return done("Invalid url: Missing tile coordinates. x", params.x);
        if (params.y == undefined)
            return done("Invalid url: Missing tile coordinates. y", params.y);
    ...
    lat_to_tile_y: function (lat, zoom) {
        return Math.floor(
            ((1 -
                Math.log(
                    Math.tan(mile.deg_to_rad(lat)) +
                        1 / Math.cos(mile.deg_to_rad(lat))
                ) /
                    Math.PI) /
                2) *
                Math.pow(2, zoom)
    ...
    _isOutsideExtent: function (options) {
        ...
        // get bboxes
        var tile_bounding_box = mercator.xyz_to_envelope(
            parseInt(coords.x),
            parseInt(coords.y),
            parseInt(coords.z),
            false
    ...
    getUploadStatus: function (options, done) {
        mile.GET(
            mile.routes.base + mile.routes.upload_status,
            options,
            function (err, json) {
    ...
    error: {
        missingInformation: function (res, missing) {
            ...
            var error_description =
                missing +
                " Check out the documentation on https://docs.systemapic.com.";
            res.json({
                error: error,
